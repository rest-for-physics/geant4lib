...
enum class EnergyDistributionFormulas { COSMIC_NEUTRONS, COSMIC_GAMMAS, FISSION_NEUTRONS_U238, ENVIRONMENTAL_GAMMAS };
...
class TRestGeant4PrimaryGeneratorInfo {
   ...
   private:
    ...
    /// \brief A 3d-std::vector with the angles, measured in degrees, of a XYZ rotation applied to the virtual
    /// generator. This rotation is used by virtualWall, virtualCircleWall and virtualCylinder generators.
   ...
   public:
    ...
    /// \brief Returns the name of the GDML volume where primary events are produced. This value has meaning
    /// only when using volume or surface generator types.
    inline TString GetSpatialGeneratorFrom() const { return fSpatialGeneratorFrom; }

    /// \brief Returns a 3d-std::vector with the position of the primary event generator. This value has
    ...
    /// \brief Returns the main spatial dimension of virtual generator. It is the size of a  virtualBox.
